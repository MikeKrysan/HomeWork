...
class RatingDonutView @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null): View(context, attributeSet) {
    ...
    //У нас есть поля для краски, и надо их инициализировать, для этого напишем специальный метод initPaint()
    ...
    //специальный метод для выбора цвета краски. Цвет у нас будет менятся от уровня рейтинга:
    ...
    //При создании нашего класса мы должны все это инициализировать, плюс мы должны получить значения из атрибутов, которые мы могли создать в XML, делаем это в блоке init:
    init {
    ...
    //Теперь нужно просчитать все размеры. Делаем мы это с помощью методов onSizeChanged(), onMeasure(), chooseDimension()
    ...
    //специальный метод для отрисовки наших арок, ведь прогресс у нас от 0 до 100, а круг 360 градусов, поэтому сконвертируем эти значения:
    ...
    //отрисовываем наше View в переопределенном методе onDraw:
    ...
    //Реализуем метод, чтобы мы могли из кода задать рейтинг:
...
/*
Чтобы устанавливать значения из XML, нам нужно сделать привязку, это мы можем сделать в папке values, либо в отдельном файле,
либо в уже имеющемся файле styles.xml. Я выберу второй вариант и напишу в этом файле следующее:

<declare-styleable name="RatingDonutView">
